...
export function HeroSection({ ip, streams, loading, videoSrc = '/spawn.mp4' }: Props) {
  ...
  const copy = () => { try { navigator.clipboard.writeText(ip); } catch { /* */ } setCopied(true); setTimeout(() => setCopied(false), 1800); };

  return (
    <header id="hero" style={{
      position: 'relative', minHeight: '92vh', overflow: 'hidden', display: 'flex', flexDirection: 'column',
      alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '80px 20px 200px',
      ...
      {/* soleil */}
      <div style={{ position: 'absolute', top: 60, right: '9%', width: 88, height: 88, background: '#f6d96b', border: `5px solid ${C.gold}`, boxShadow: 'inset -5px -5px 0 rgba(224,164,53,.5), 0 0 60px rgba(246,217,107,.7)', animation: 'v-spinSlow 50s linear infinite' }} />
      {/* nuages */}
      <div style={cloud(90, 160, 48, 0.92, 60, 0, '0 18px 0 -6px #fff,40px 18px 0 -6px #fff,-40px 18px 0 -6px #fff')} />
      ...
      <div style={{ position: 'relative', zIndex: 10, width: '100%', maxWidth: 1220, display: 'flex', gap: 36, alignItems: 'center', justifyContent: 'center', flexWrap: 'wrap' }}>
        <div style={{ flex: '1 1 380px', maxWidth: 560, textAlign: 'left' }}>
          ...
          <p style={{ fontFamily: FONT.pixel, fontSize: 'clamp(20px,3vw,30px)', color: C.woodDark, margin: '6px 0 8px', textShadow: '2px 2px 0 rgba(255,255,255,.5)' }}>Le serveur Minecraft des VTubers FR</p>
          <p style={{ fontSize: 'clamp(15px,1.8vw,19px)', color: '#4a3a25', maxWidth: 620, margin: '0 0 30px', lineHeight: 1.55 }}>Une aventure RPG médiévale épique sur le modpack <strong>Cisco's Medieval RPG · Dragonfyre</strong> enrichi de dizaines de mods custom. Magie, donjons, dragons et chaos communautaire.</p>
          ...
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 14, background: C.parchment, border: `4px solid ${C.woodDark}`, borderRadius: 5, padding: '10px 16px', boxShadow: 'inset -4px -4px 0 rgba(58,42,24,.2), inset 4px 4px 0 rgba(255,255,255,.5), 0 5px 0 rgba(58,42,24,.35)' }}>
            <span style={{ fontFamily: FONT.arcade, fontSize: 10, color: C.wood }}>IP</span>
            <span style={{ fontFamily: FONT.pixel, fontSize: 22, fontWeight: 600, color: C.ink, letterSpacing: 1 }}>{ip}</span>
            <button onClick={copy} className="v-btn" style={{ cursor: 'pointer', fontFamily: FONT.pixel, fontSize: 15, color: '#fff', padding: '6px 12px', background: C.grass, border: `3px solid ${C.grassBorder}`, borderRadius: 4, boxShadow: 'inset -2px -2px 0 rgba(0,0,0,.25), inset 2px 2px 0 rgba(255,255,255,.3)' }}>{copied ? 'Copié ✓' : 'Copier'}</button>
          </div>
        </div>

        {/* fenêtre : stream à la une */}
        <div style={{ flex: '1 1 380px', maxWidth: 560, width: '100%', position: 'relative', background: `linear-gradient(${C.woodBar1}, ${C.woodBar2})`, border: `6px solid ${C.woodDark}`, borderRadius: 9, padding: 14, boxShadow: 'inset -5px -5px 0 rgba(0,0,0,.3), inset 5px 5px 0 rgba(255,255,255,.18), 0 10px 0 rgba(58,42,24,.45)' }}>
      ...
      {/* torches */}
      <div style={{ position: 'absolute', bottom: 96, left: '7%', zIndex: 9, width: 14, height: 54, background: C.wood, border: `3px solid ${C.woodDark}` }} />
      <div style={{ position: 'absolute', bottom: 140, left: 'calc(7% - 6px)', zIndex: 9, width: 24, height: 24, background: 'radial-gradient(circle,#ffe066,#e0772c)', borderRadius: '50%', boxShadow: '0 0 24px rgba(224,119,44,.9)', animation: 'v-flick .4s ease-in-out infinite' }} />
      <div style={{ position: 'absolute', bottom: 96, right: '7%', zIndex: 9, width: 14, height: 54, background: C.wood, border: `3px solid ${C.woodDark}` }} />
      <div style={{ position: 'absolute', bottom: 140, right: 'calc(7% - 6px)', zIndex: 9, width: 24, height: 24, background: 'radial-gradient(circle,#ffe066,#e0772c)', borderRadius: '50%', boxShadow: '0 0 24px rgba(224,119,44,.9)', animation: 'v-flick .45s ease-in-out infinite', animationDelay: '-.2s' }} />
    </header>
  );
}
